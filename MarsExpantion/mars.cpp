...
int bitsCombine(int left, int right){
	...
	return ((left & 255)+(left &(255<<8)) + (right & 255)+(right &(255<<8)))& ((1<<8)-1);
}


WORD MDC4TypeSbox(WORD input , static WORD *s_box){
	WORD R=(input&((1 <<(W/8))-1));
	WORD L =(input >> (W/8)) & ((1 <<(W/8))-1);
	int index;

	WORD tmpL=s_box[bitsCombine((s_box[L] & (((1 << (W / 4)) - 1))) , (s_box[R] & (((1 << (W / 4)) - 1))))];
	R=s_box[bitsCombine((s_box[L] >> (W/4)) & ((1 <<(W/4))-1) , (s_box[R] >> (W/4)) & ((1 <<(W/4))-1))]  ; 
	L=tmpL;
