...
WORD MDC4TypeSbox(WORD input , int round, static WORD *s_box){
	WORD R=(input&((1ULL <<(64/4))-1));
	WORD L=(input&(((1ULL <<(64/4))-1)<<(64/4)))>>(64/4);
	int index;

	for(index=0;index<round; index++){
		L=s_box[bitsCombine((s_box[L] & (((1 << (64 / 4)) - 1))) , (s_box[R] & (((1 << (64 / 4)) - 1))))];
		R=s_box[bitsCombine((s_box[L] & (((1 << (64 / 4)) - 1)<<(64/4))) , (s_box[R] & (((1 << (64 / 4)) - 1)<<(64/4))))]  ; 
	}

	return L<<(W/2)|R;
}

int bitsCombine(int left, int right){
	return ((left & 255)+(left &(255<<8)) + (right & 255)+(right &(255<<8)))& ((1<<16)-1);
}
